  ...
  {{-- <form action="{{ route('admin-submitskor') }}" method="POST"> --}}
                ...
                <table class="dt-fixedheader table">
                    ...
                    <tbody>
                        @foreach ($dataSPIP as $dSPIP)
                            <tr>
                                <th>{{ $dSPIP->unsur }}</th>
                                <form id="skorForm-{{ $dSPIP->id }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                    <th>
                                        <div>
                                            <input type="text" class="form-control" id="skorInput-{{ $dSPIP->id }}" placeholder="Skor ...." name="skor" value="{{ old('skor', $dSPIP->skor) }}" />
                                        </div>
                                    </th>
                                    <th>
                                        <button type="button" class="btn btn-success" onclick="submitSkor({{ $dSPIP->id }})">
    ...
    <script type="text/javascript">
        function submitSkor(id) {
            ...
            $.ajax({
                url: "{{ url('admin/submitskorSPIP') }}/" + id,  // URL untuk mengirim data
                method: "PUT",
                data: {
